...
import { Mail, Phone, MessageCircle, ChevronDown, ChevronUp, CheckCircle, MapPin, Clock } from 'lucide-react';
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-16 space-y-16">
      <div className="text-center">
        <h1 className="text-4xl font-bold text-foreground mb-4">Liên hệ &amp; Hỗ trợ</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="space-y-5">
          <h2 className="text-base font-semibold text-foreground">Thông tin liên hệ</h2>
          {[
            { icon: Mail, label: 'Email hỗ trợ', value: 'support@smartbox-elogistics.vn', sub: 'Phản hồi trong 24h' },
            { icon: Phone, label: 'Hotline khẩn cấp', value: '1800 SMART BOX', sub: '24/7 · Miễn phí' },
            { icon: MessageCircle, label: 'Live Chat', value: 'Zalo / Teams', sub: 'Giờ hành chính' },
            { icon: MapPin, label: 'Địa chỉ', value: 'TP. Hồ Chí Minh, Việt Nam', sub: 'Văn phòng chính' },
            { icon: Clock, label: 'Giờ làm việc', value: 'T2 - T6: 8:00 - 17:30', sub: 'GMT+7' },
          ].map(item => (
            <div key={item.label} className="flex gap-3 p-4 bg-card border border-border rounded-xl">
        ...
        <div className="lg:col-span-2">
          <div className="bg-card border border-border rounded-2xl p-7">
            <h2 className="text-base font-semibold text-foreground mb-6">Gửi yêu cầu hỗ trợ</h2>
            {submitted ? (
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <h3 className="text-base font-semibold text-foreground mb-2">Gửi thành công!</h3>
                <p className="text-sm text-muted-foreground max-w-xs">
                  Chúng tôi sẽ phản hồi trong vòng 24 giờ làm việc tới địa chỉ email của bạn.
                </p>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <Field label="Họ và tên">
                    <Input value={form.name} onChange={e => set('name', e.target.value)} className="bg-secondary border-border" placeholder="Nguyễn Văn A" required />
                  </Field>
                  <Field label="Email">
                    <Input type="email" value={form.email} onChange={e => set('email', e.target.value)} className="bg-secondary border-border" placeholder="email@company.com" required />
                  </Field>
                </div>
                <Field label="Chủ đề">
      ...
      <div>
        <h2 className="text-2xl font-bold text-foreground mb-8 text-center">Câu hỏi thường gặp</h2>
